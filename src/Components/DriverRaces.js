import React, {useEffect, useState} from 'react';
import {
  View,
  Text,
  FlatList,
  Button,
  TouchableOpacity,
  ActivityIndicator,
  Alert,
  BackHandler,
} from 'react-native';

import axios from 'axios';

import styles from '../styles';

import {store} from '../Redux/reducer';
import {setCurrentRacesPage} from '../Redux/actions';
import {TextInput} from 'react-native-gesture-handler';

const DriverRaces = ({navigation}) => {
  const [isLoaded, setIsLoaded] = useState(false);
  const [currentRaces, setCurrentRaces] = useState([]);
  const [currentPage, setCurrentPage] = useState(
    store.getState().currentRacesPage,
  );
  const offset = 10;
  const limit = 10;
  const getData = async () => {
    try {
      setIsLoaded(false);
      const response = await axios.get(
        `https://ergast.com/api/f1.json?limit=${limit}&offset=${
          offset * currentPage
        }`,
      );
      setCurrentRaces(await response.data.MRData.RaceTable.Races);
      setIsLoaded(true);
    } catch {
      Alert.alert(
        'Неполадки в сети',
        'Проверьте интернет соединение и перезапустите приложение',
        [{text: 'OK', onPress: () => BackHandler.exitApp()}],
        {cancelable: false},
      );
    }
  };

  useEffect(() => {
    store.dispatch(setCurrentRacesPage(currentPage));
    getData();
    return () => {};
  }, [currentPage]);

  if (isLoaded === true) {
    return (
      <View style={styles.main}>
        <View style={styles.list}>
          <FlatList
            data={currentRaces}
            renderItem={({item}) => {
              return (
                <View style={styles.listItem}>
                  <View
                    style={{
                      justifyContent: 'space-between',
                      flexDirection: 'row',
                    }}>
                    <Text>
                      {item.raceName} {item.season}
                    </Text>
                    <Text> {item.Circuit.Location.country}</Text>
                  </View>
                </View>
              );
            }}
            keyExtractor={(item, i) => `${i}`}></FlatList>
        </View>
        <View style={styles.pages}>
          <Button
            title="Назад"
            color={currentPage >= 1 ? '' : 'grey'}
            onPress={() =>
              currentPage >= 1
                ? setCurrentPage(currentPage - 1)
                : console.log('No')
            }></Button>
          <Text>{currentPage + 1}</Text>
          <Button
            title="Дальше"
            color={currentRaces.length == 10 ? '' : 'grey'}
            onPress={() =>
              currentRaces.length == 10
                ? setCurrentPage(currentPage + 1)
                : console.log('No')
            }></Button>
        </View>
      </View>
    );
  } else {
    return (
      <View style={styles.loading}>
        <ActivityIndicator color="lime" size={100}></ActivityIndicator>
      </View>
    );
  }
};

export default DriverRaces;
